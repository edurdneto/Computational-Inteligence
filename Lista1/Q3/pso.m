...
for i=1:numCycles
    ...
    values = evaluateSolutions(particles,X);
...
ypred=polyval(bestGlobalPosition,X(:,1));

figure; plot(X(:,1),X(:,2),'bo'); hold on; grid; % diagrama de dispersao
xlabel('Velocidade do vento [m/s]'); 
ylabel('Potencia gerada [kWatts]');

plot(X(:,1),ypred,'r-'); % Sobrepoe curva de regressao aos dados
hold off;
